								...
								<table class="table table-bordered">
									...
									<tbody>
										...
										<tr @if(isset($folder_detail)) @if($folder_detail->id==$row_folder->id) style="background: aliceblue;" @endif @endif>
											...
											<td>{{$row_folder->sessionimage->title}}</td>
											<td>{!! $row_folder->sessionimage->description !!}</td>
											<td>
												@php $folder_id = base64_encode($row_folder->id.'&'.auth::user()->user_name); @endphp
												<!--a href="{{route('account.folders.chat', $folder_id)}}" class="btn btn-secondary">Go to chat</a-->
										    </td>
										</tr>
										@empty
										<tr>
