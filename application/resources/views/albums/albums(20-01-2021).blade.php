		  ...
		  $.each(data.albums, function(index, value) {
				...
				if ($(".ext"+value.id)[0]){
					...
					img.onload = function() {
						...
						html = '<figure class="album-box ext'+data.value.id+'" itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject"> <a href="'+url+'" itemprop="contentUrl" data-size="'+image_dimension+'"> <img src="'+url+'" itemprop="thumbnail" alt="'+data.value.title+'"  style="width:'+w+'px; height:'+h+'px;" /> </a> <figcaption itemprop="caption description" class="_figcaptiondescription" style="width:'+w+'px;"> <p class="img-title">'+data.value.title+'</p> <div class="_description">'+ data.value.description +'</div> <p class="plus-icon toCart addtocartbtn" style="color:#fff;" data-val="'+ data.value.id +'"><img src="{{asset("content/images/folder-plus1.png")}}"> Add to Folder</p> </figcaption> </figure>';
						//console.log(html);
